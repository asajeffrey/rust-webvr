...
impl VRFutureFrameData {
    ...
    pub fn block(&mut self) -> VRFrameData {
        let result = match *self {
            VRFutureFrameData(State::Resolved(ref result)) => return result.clone(),
            VRFutureFrameData(State::Blocked(ref recv)) => recv.recv().expect("Failed to get frame data"),
        };
        self.0 = State::Resolved(result.clone());
        result
    }
}
